...
void ofxPTAMM::init(int imgW, int imgH, string configFile) {
	...
	GVars3::GUI.StartParserThread(); // Start parsing of the console input
	atexit(GVars3::GUI.StopParserThread); 
    // First, check if the camera is calibrated.
    // If not, we need to run the calibration widget.
	TooN::Vector<NUMTRACKERCAMPARAMETERS> vTest;

	vTest = GVars3::GV3::get<TooN::Vector<NUMTRACKERCAMPARAMETERS> >("Camera.Parameters", PTAMM::ATANCamera::mvDefaultParams, GVars3::HIDDEN);
	
    camera = new ofxATANCamera("Camera");
    camera->loadParameters(configFile);

	if(vTest == PTAMM::ATANCamera::mvDefaultParams)
	{
		cout << endl;
