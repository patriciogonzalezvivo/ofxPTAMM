...
namespace TooN{
	namespace Internal{
		...
		///@ingroup gInternal
		template<class F, class Precision, int Size> std::pair<Vector<Precision,Size>, Vector<Precision, Size> > extrapolate_to_zero_J(F& f, int size)
		{
			using std::isfinite;
			using std::max;
			using std::isnan;
			...
			*/
			...
			vector<vector<Precision> > P_i(Vector<Precision,Size>(Ones(size)));
			vector<vector<Precision> > P_i_1(Vector<Precision,Size>(Zeros(size)));
			...
			for(int i=0; i < Npts; i++)
			{
				...
				Vector<bool, Size> better(Zeros(size)); //Did we get better?
				...
				for(int k=0; k < best_err.size(); k++)
				{
					...
					for(int j=1; j <= i; j++)
					{
						...
						P_i[k][j] = (cj * P_i[k][j-1] - P_i_1[k][j-1]) / (cj - 1);

						if(any_ok || isfinite(P_i[k][j]))
							ever_ok[k] = 1;
				...
				bool all_bad=1
				for(int k=0; k < best_err.size(); k++)
					if(ever_ok[k] && !better[k] && i > 0 && (abs(P_i[k][i] - P_i_1[k][i-1]) > t * best_err[k]|| isnan(P_i[k][i])))
					{
						//Still bad
					}
					else
	...
	*/
	...
	template<class F, int Fsize, int S, int P, class B> Matrix<> numerical_Jacobian(const F& f, int fsize, const Vector<S, P, B>& x)
	{
		using namespace Internal;
