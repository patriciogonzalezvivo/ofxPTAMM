...
ATANCamera::ATANCamera(string sName){
  // The camera name is used to find the camera's parameters in a GVar.
    msName = sName;
    mgvvCameraParams = new TooN::Vector<5>;
...
ATANCamera::ATANCamera(string sName, ImageRef irSize, Vector<NUMTRACKERCAMPARAMETERS> vParams){
    ...
    //(*mgvvCameraParams) = vParams;
    if (mgvvCameraParams != NULL)
        delete mgvvCameraParams;
    mgvvCameraParams = new TooN::Vector<5>;
    *mgvvCameraParams = TooN::makeVector(vParams[0], vParams[1], vParams[2], vParams[3], vParams[4]);
